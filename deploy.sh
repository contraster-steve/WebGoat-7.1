...
/usr/bin/java -Dfile.encoding=UTF-8 -Dserver.port=5005 -Dserver.address=localhost -Dhsqldb.port=9001 -javaagent:./webgoat-standalone/target/contrast.jar -Dcontrast.config.path=./webgoat-standalone/target/contrast_security.yaml -jar ./webgoat-container/target/webgoat-container-7.1-war-exec.jar </dev/null &>/dev/null &
sleep 25
echo "Curl URLS"
